...
Observing the histograms, the cargo loss distribution is notably wider than the equipment failure distribution characterised by tighter and stable curves. 

![images](agg_loss_distr_cl.png)

This suggests that the gap between the mean and the 99th percentile is large with the risk profile being more tail-heavy. Extreme heat also has a relatively small rightward shift compared to intense shock events like the solar flare scenario. The distribution here creates important implications for pricing where capital allocation and risk loading needs to account for higher tail risk significance. 
...
The code used for this task can be seen below: 

![images](Setting_up_Q4.png)
![images](Simulations_Q4.png)
...
The number of claims filed under worker compensation were much lower than some other hazard areas such as business interruption, and had a much lower average severity. This can be attributed due to individual policies having much lower premiums and compensation than those catering towards entire businesses, as well as a bulk of reported issues revolving directly around how the mining business itself is affected. Workers in the Helionis system reported a lower total number of claims, but were higher in average severity of claim, indicating that accidents occurring on Helionis mining systems were typically less frequent, but greater in magnitude. 

The coefficients of the model were calculated for claim counts, across all 3 solar systems. Increase in exposure had the highest effect on claim count increase, which is due to longer policies incurring more claims. A similar model was created for claim amounts. Safety training index and protective gear quality were two factors that greatly contributed to lower claim amounts, by mitigating the damage caused in an incident.
