...
### Cargo Loss: 
![images](Cargo_loss_agg_losses.png)
...
![images](equipment_data_exploration.png)

The variation in risk profiles for each solar system aligns with their environmental and operational characteristics. At a high level, Helion Cluster stands out as a frequency driven risk profile with the lowest exposure but highest claim occurrence. The highest premium per unit of exposure is attributed to recurring, lower-severity operational losses that is common in debris-heavy environments. Conversely, Zeta has a severity driven risk profile with the highest average amount per claim. This aligns with its extreme and volatile conditions (volcanic environments), legitimizing high cost failures which drives peak total severity. Epsilon represents a balance of these extremes, with a combination of environmental stressors (weather variation, corrosive atmosphere) and operational exposure (radiation) with equipment failure evenly distributed across both dimensions. 

Upon fitting policy variables into a frequency model (Poisson GLM) and severity model (Gamma GLM), core drivers of frequency and severity can be identified. Exposure is highly significant in the frequency model (p < 2e-16), suggesting that larger exposure increases the opportunity for claims but is irrelevant to claim size (p > 0.5 in Gamma GLM) [L fig.12]. Operational factors including usage intensity and equipment age are strong drivers to both claim occurrence and severity (p < 2e-16). Uniquely, maintenance intensity is a small but statistically significant influencer of claim occurrence [L fig.13]. Claim severity is influenced by equipment type where Quantum Bore exhibits ~31% higher claim size and FexStram Carrier exhibits 46% lower severity. 

The distribution of key variables mentioned above are broadly similar across solar systems with equipment composition being nearly identical and no distribution shifts for other drivers 		[L fig.16].

A worst-case scenario such as a solar flare is expected to increase both claim frequency and severity due to radiation-induced equipment failures [L fig.8]. Its impact is varied by solar system, starting with Epsilon which experiences an 83% increase in aggregate losses driven by significant increase in claim likelihood (1.4x) justified by volatile environments (operations under a fast-burning star) and costly repair escalation (1.3x) when conditions worsen. Zeta also shows significant uplift (49%), driven by moderate exposure to solar flare activity and extreme conditions (volcanic activity) which is predisposed to operational disruptions. Helionis experiences minimal loss uplift (16%), reflecting its predictable and stable solar conditions which cushion the extent of disruption. 
...
![images](cargo_data_exploration.png)

Compared to other hazards, cargo losses show an aggregated risk profile with no segmentation by solar system. The lack of segmentation suggests that cargo losses are more dependent on intrinsic factors of the cargo/shipment itself as opposed to local operating conditions. At a high level, cargo loss has a relatively higher claim rate of 48% which indicates frequent losses. This is escalated by a significant average severity of $7.8m where cargo incidents carry larger financial vulnerabilities due to the large scale of the operation. 

While frequency of cargo losses could be captured using a Poisson GLM, the presence of over dispersion indicates that claim occurrence exhibits significant variability. Thus, a Negative Binomial model was utilised to account for the excess volatility through a dispersion parameter, accounting for real-world cargo operations where claims are impacted by unobserved factors including route disruptions and shocked conditions [L fig.14]. The full frequency model output incorporated a wide range of variables but only route_risk was identified as a significant driver (p < 0.0005). This suggests that variability in claim occurrence is not driven by shipment characteristics but unobserved events and stochastic behaviour [L fig.15]. 

However, the Gamma GLM for claim severity demonstrates variable insights with intrinsic cargo factors like cargo value, cargo type driving claim amount. Cargo types with high-value commodities including gold and platinum emerge as the biggest contributor to large losses with cargo value following closely behind, indicating how financial exposure is embedded in each shipment. Exterior variables such as route risk and solar radiation further exacerbate claim amount, indicating an impact to not only frequency but magnitude of loss. With a severity standard deviation that is 3.2x the mean, this indicates a highly skewed loss distribution characterised by rare but extreme events, urging a price modelling approach that needs to place appropriate risk loadings and features to control risk exposure. 

Extreme scenarios were similarly modelled but stress factors were uniquely applied to known variables that were directly exposed to environmental shocks [L fig.11]. For a solar flare scenario, a 2x spike on solar_radiation was applied to reflect the significant spike in radiation exposure which is capable of impacting cargo integrity and disrupting navigation systems. A 1.5x increase in route_risk was incorporated to capture operational disruptions that weren’t fully represented such as rerouting and unknown transit risk. For extreme heat, more mild multipliers of 1.4x and 1.1x were applied respectively to demonstrate a more gradual and incremental impact on transport integrity which are less likely to trigger a system-wide disruption as a solar flare. 
...
A Poisson GLM was used to model claim frequency, and a Gamma GLM used to model claim severity, with their coefficients shown in [M fig.8] and [M fig.9] respectively. The risk of each scenario was calculated by first building these models for frequency and severity respectively. Through analysis of coefficients in the Poisson model [M fig.8], an increase in exposure resulted in the biggest increase in claim count, which is expected, as policies that exist over a longer time period would naturally have more claims. Conversely, an increase in average crew experience and maintenance frequency would result in lower claim counts, highlighting the importance of proficiency and equipment quality in foreign, hostile environments. The Gamma model [M fig. 9], shows production load as having a negative impact on claim amounts. This is attributable to stations with higher production load naturally being better equipped to handle it, requiring more rigorous standards of safety and protocol that decrease the severity and likelihood of business interruption

Through the table shown in [M fig.10], it is clear that the Helionis system experienced a much lower number of business interruption claims and total exposure than Zeta and Epsilon, which each experienced similar levels of both. However, the Helionis system saw the claims with the highest average severity, nearly 10% greater than the next highest, and consequently, highest annual premiums. Additionally, their claim frequency was  comparable to other systems. This indicates that the number of disruptive events, such as solar flares, is lower due to Helionis’ low flare and wind activity, but that each event causes damage comparable to those in other solar systems. 


The aggregate losses for each risk scenario are shown in [M fig.11] The Epsilon system experienced the most noticeable jump in loss between scenarios, with the baseline loss almost doubling under extreme heat. Business operations in the Epsilon system must be highly susceptible to heat, and therefore the insurance policy should account for this by raising premiums, as well as introducing additional charges for this specific occurrence. Aggregate loss in the Helionis system did not change significantly between the 3 levels, and remained low in comparison. As discussed earlier, this can be attributed to the lower number of claims reported overall in the Helionis region, meaning that standard business procedures are not at high risk of disruption. The Zeta system’s losses were close to Epsilon’s at baseline, but grew at a much smaller rate, indicating that environmental incidents impact them at a lesser rate, which could be due to the advanced protection technology active within the Zeta system. 
...
The number of claims filed under worker compensation (shown in [M fig.12]) were much lower than some other hazard areas such as business interruption, and had a much lower average severity. This can be attributed due to individual policies having much lower premiums and compensation than those catering towards entire businesses, as well as a bulk of reported issues revolving directly around how the mining business itself is affected. Workers in the Helionis system reported a lower total number of claims, but were higher in average severity of claim, indicating that accidents occurring on Helionis mining systems were typically less frequent, but greater in magnitude. 

The coefficients of the model were calculated and displayed in [M fig.13] for claim counts, across all 3 solar systems. Increase in exposure had the highest effect on claim count increase, which is due to longer policies incurring more claims. A similar model was created for claim amounts, and can be seen in [M fig.14]. Safety training index and protective gear quality were two factors that greatly contributed to lower claim amounts, by mitigating the damage caused in an incident.

The graphs in [M fig.15] show that sprains and cuts cost the most in total over all 3 solar systems. It can be noted that stress played a bigger factor in the Helionis and Zeta systems than Epsilon, suggesting that workers find those environments or work stations more hostile. Possibilities of hostile work environments are further enhanced by the injury cause graphs [M fig.16], which show that violence incurs a significant cost in the Zeta system compared to the other 2 solar systems.
